...
const INDEX_URL = process.env.FINGERPRINT_INDEX_URL || process.env.BLOB_FINGERPRINT_URL;
    ...
    loadPromise = (async () => {
        ...
        if (INDEX_URL) {
            try {
                ...
                console.log(`✅ Fingerprint index loaded from URL: ${data.total} cards (generated: ${data.generated_at})`);
            }
            catch (error) {
                console.warn(`⚠️ Failed to fetch fingerprint index from URL (hasUrl=${Boolean(INDEX_URL)}). Falling back to local file.`, error);
            }
        }
        else {
            console.warn(`⚠️ FINGERPRINT_INDEX_URL is not set (isVercel=${IS_VERCEL}). Falling back to local file.`);
        }
        if (!data) {
            try {
                ...
                data = JSON.parse(raw);
                console.log(`✅ Fingerprint index loaded from file: ${data.total} cards (generated: ${data.generated_at})`);
            }
            catch (error) {
...
function scoreEntry(query, candidate) {
 ...
 */
export async function findBestVisualMatch(query, topK = 5) {
...
function buildSearchIndices(entries) {
    const byNameKey = new Map();
    const bySetCn = new Map();
    ...
    console.log(`📑 Built search indices: ${byNameKey.size} name keys, ${bySetCn.size} set+cn pairs`);
 ...
 */
export async function findBestVisualMatchAdvanced(query, topK = 5, ocr, oracleIds) {
    const entries = await loadIndex();
    ...
    let candidates = oracleIds && oracleIds.length > 0
    ...
    const pool = ocrPool && ocrPool.length > 0 && ocrPool.length <= 200
        ? ocrPool
        : candidates;
    ...
    if (ocrPool &&
        ...
        const fallback = candidates.map((entry) => ({
            ...scoreEntry(query, entry),
            entry,
        }));
