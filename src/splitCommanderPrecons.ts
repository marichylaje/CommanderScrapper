...
const INPUT_FILE = './data/commander_precons.json'; // ruta original
const OUTPUT_DIR = './data/commanders/'; // carpeta donde guardar los subarchivos
const CHUNK_SIZE = 5;
...
function main() {
  ...
  chunks.forEach((chunk, index) => {
    ...
    const outPath = path.join(OUTPUT_DIR, `commander_precons_${suffix}.json`);
    fs.writeFileSync(outPath, JSON.stringify(chunk, null, 2), 'utf-8');
    console.log(`✅ Generado: commander_precons_${suffix}.json con ${chunk.length} decks`);
  });
