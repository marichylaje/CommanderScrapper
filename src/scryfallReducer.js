...
async function fetchScryfallMetadata() {
    ...
    if (!res.ok)
        throw new Error('❌ Error al obtener metadata de Scryfall.');
    return (await res.json());
}
async function fetchBulkJson(downloadUri) {
    const res = await fetch(downloadUri);
    if (!res.ok)
        throw new Error('❌ Error al descargar bulk JSON.');
    return (await res.json());
}
function reduceCard(card) {
    ...
    const face_flavor_names = Array.isArray(faces)
        ? faces.map(f => f.flavor_name).filter((s) => !!s && s.trim().length > 0)
        : [];
...
async function main() {
    try {
        ...
        const filteredData = rawData.filter((card) => !card.type_line?.startsWith('Token Creature'));
        ...
        for (const oracleId of Object.keys(byOracle)) {
            ...
            const pickByFlavor = new Map(); // key normalizada → doc
