...
async function main() {
  ...
  const pickHitsFor = (q: string) => {
    const qn = normalize(q);
    return docs.filter((d) =>
      [d?.name, d?.face_name, d?.flavor_name].some(v => normalize(v ?? '') === qn)
    );
  };
  ...
  const rows = QUERIES.map((q) => {
    ...
    const via = hits.map((h) => {
      const qn = normalize(q);
      if (normalize(h?.name ?? '') === qn) return 'name';
      if (normalize(h?.face_name ?? '') === qn) return 'face_name';
      if (normalize(h?.flavor_name ?? '') === qn) return 'flavor_name';
      return 'unknown';
    });
    return {
      query: q,
