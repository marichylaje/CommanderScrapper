...
const normalize = (value) => (value ?? '')
    ...
    .normalize('NFKC')
    .trim();
function firstFaceImage(card, key) {
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const ranked = await rankCandidatePrintings(payload.photoBuffer, printings.map((card) => ({
            artImageUrl: card?.image_uris?.art_crop ??
                firstFaceImage(card, 'art_crop'),
            card,
            fullImageUrl: card?.image_uris?.normal ??
