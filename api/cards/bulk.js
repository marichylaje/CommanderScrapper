...
export default async function handler(req, res) {
    ...
    const normalize = (str) => (str ?? '').toLowerCase().replace(/[\u2019’']/g, "'").normalize('NFKC').trim();
    // Parsear las consultas entrantes
    const parsedQueries = rawNames.map((name) => {
        if (name.includes('|')) {
        ...
        try {
            ...
            for (let j = 0; j < batch.length; j++) {
                ...
                if (query.set && query.cn) {
                    const exactPrint = hits.find((hit) => normalize(hit.document.name) === normalize(query.cleanName) &&
                ...
                if (!resolvedDoc) {
                    ...
                    const nameMatch = hits.find((hit) => normalize(hit.document.name) === normalize(query.cleanName));
                    if (nameMatch) {
                        resolvedDoc = nameMatch.document;
                    }
                    else {
                        // Coincidencia de face_name
                        const faceMatch = hits.find((hit) => normalize(hit.document.face_name ?? '') === normalize(query.cleanName));
                        if (faceMatch) {
                            resolvedDoc = faceMatch.document;
                        }
                        else {
                            // Coincidencia de flavor_name
                            const flavorMatch = hits.find((hit) => normalize(hit.document.flavor_name ?? '') === normalize(query.cleanName));
                            if (flavorMatch) {
                                resolvedDoc = flavorMatch.document;
                            }
                            else {
                                // Coincidencia de face_flavor_names
                                const ffMatch = hits.find((hit) => {
                                    const arr = hit.document.face_flavor_names ?? [];
                                    return (Array.isArray(arr) &&
                                        arr.some((s) => normalize(s) === normalize(query.cleanName)));
                                });
                                if (ffMatch) {
