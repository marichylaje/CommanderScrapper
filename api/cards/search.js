    ...
    try {
        ...
        if (isCommander) {
            try {
                ...
                if (cn) {
                    hits = hits.filter((doc) => doc.collector_number === cn);
                }
        ...
        if (hits.length === 0) {
            try {
                ...
                const scryfallRes = await fetch(`https://api.scryfall.com/cards/search?q=${encodeURIComponent(scryfallQuery)}&page=${page}`);
                if (scryfallRes.ok) {
                    const scryfallData = (await scryfallRes.json());
                    if (scryfallData?.data) {
                        hits = scryfallData.data.filter((doc) => doc && doc.id && doc.object !== 'error');
                        if (cn) {
                            hits = hits.filter((doc) => doc.collector_number === cn);
                        }
        ...
        if (isCommander)
            nextUrlParams.append('is_commander', 'true');
        if (set)
            nextUrlParams.append('set', set);
        if (cn)
            nextUrlParams.append('cn', cn);
