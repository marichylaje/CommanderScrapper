...
export default async function handler(req, res) {
    try {
        ...
        const exactMatch = results.hits?.find((hit) => normalize(hit.document.name) === normalize(name));
        if (exactMatch) {
        ...
        const faceMatch = fallbackResults.hits?.find((hit) => normalize(hit.document.face_name ?? '') === normalize(name));
        if (faceMatch) {
        ...
        const flavorMatch = flavorResults.hits?.find((hit) => normalize(hit.document.flavor_name ?? '') === normalize(name));
        if (flavorMatch) {
        ...
        const ffMatch = faceFlavorResults.hits?.find((hit) => normalize((hit.document.face_flavor_names ?? []).join('|')).split('|')
